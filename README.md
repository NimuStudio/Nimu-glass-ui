...
Nimu Glass UI 是一套**完整的 Web UI 体系**：玻璃拟态（Glassmorphism）+ 浅色 + 深色三主题一键切换，附带液态玻璃背景、光标泛光、玻璃边缘高光等特效。
...
👉 [https://NimuStudio.github.io/Nimu-glass-ui/](https://NimuStudio.github.io/Nimu-glass-ui/)（GitHub Pages，配置方法见文末）
...
> **GitHub Pages 开启方法**：仓库 Settings → Pages → Source 选 `Deploy from a branch` → Branch 选 `main` / `/ (root)` → Save，即可通过 `https://NimuStudio.github.io/Nimu-glass-ui/` 访问在线演示。
...
> 想支持这个项目？☕ [去爱发电请我喝杯咖啡](https://www.ifdian.net/a/NimuStudio)（或通过 GitHub 主页赞助入口）。**¥18 支持者档**支持者的名字（GitHub 用户名或昵称）会永久列入上表。
>
